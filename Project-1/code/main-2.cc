...
int main (int argc, char* argv[]) {
	...
	int tNo = atoi(argv[2]); //Table Number
	int aNo = atoi(argv[3]); //Attribute Number
	...
	for (int i = 0; i < tNo; i++) {
		...
		string tName = tN; //Table Name

		int taNo = i * aNo; //Table attribute
		vector<string> atts;
		vector<string> types;
		for (int j = 0; j < taNo; j++) {
			char aN[20]; sprintf(aN, "A_%d_%d", i, j);
			string aName = aN; //Attribute Name
			atts.push_back(aN);
